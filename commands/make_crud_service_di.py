...
class Command(BaseCommand):
    ...
    def create_service_file(self, app_name, model_name):
        ...
        with open(service_file, "w") as f:
            ...
            f.write(f"    repository: {model_name}Repository\n\n")
            ...
            f.write(f"    def __init__(self, repository):\n")
    ...
    def update_di_container(self, app_name, model_name):
        ...
        lines.insert(last_line_index + 4, new_imports[2])
        ...
        # find the last line of the file that includes `, repository=` and save the index
        last_param_search = ", repository="
        last_line_index = 0
        with open(container_file_name, "r") as file:
            lines = file.readlines()
            for i, line in enumerate(lines):
        ...
        # add the new repository to the container
        new_repository = f"    {self.calc_filename(model_name)}_repository = providers.Singleton({model_name}Repository, model={self.calc_filename(model_name)}_model)\n"
        # add the new service to the container
        new_service = f"    {self.calc_filename(model_name)}_service = providers.Singleton({model_name}Service, repository={self.calc_filename(model_name)}_repository)\n"
